...
interface RequestState {
	...
	usage: {
		...
		totalTokens?: number;
		costUsd?: number;
		providerCostUsd?: number; // provider-returned cost from OpenRouter (distinct from costUsd estimate)
...
export class UsageCollector {
	...
	private async _handleEndInternal(msg: EndMessage): Promise<void> {
		...
		if (state.usage.model) {
			...
			// When the provider returned a cost via SSE or JSON, use it directly;
			// otherwise fall back to the client-side estimate. estimateCostUSD()
			// returns a literal 0 for unknown models, which resolveCostUsd maps to
			// undefined so the writer's `?? null` collapses it to null instead of
			// persisting a real 0. Capture the narrowed model so the closure keeps
			// the `if (state.usage.model)` narrowing.

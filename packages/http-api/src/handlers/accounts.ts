...
export function createOpenRouterAnthropicAccountAddHandler(
	dbOps: DatabaseOperations,
) {
	return async (req: Request): Promise<Response> => {
		try {
			...
			// Create OpenRouter Anthropic account in database
			const accountId = crypto.randomUUID();
			const now = Date.now();
			const db = dbOps.getAdapter();
			await db.run(
			...
			log.info(
				`Successfully added OpenRouter Anthropic account: ${name} (Priority ${priority})`,
			);
			...
			return jsonResponse({
				message: `OpenRouter Anthropic account '${name}' added successfully`,
				account: {
					id: account.id,
					name: account.name,
					provider: account.provider,
			...
			log.error("OpenRouter Anthropic account creation error:", error);
			if (error instanceof ValidationError) {
				return errorResponse(BadRequest(error.message));
			}
			return errorResponse(
				error instanceof Error
					? error
					: new Error("Failed to create OpenRouter Anthropic account"),
			);
		}
	};
}

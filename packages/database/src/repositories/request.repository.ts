...
export class RequestRepository extends BaseRepository<RequestData> {
	async save(data: RequestData): Promise<void> {
		...
		`,
			[
				...
				usage?.model || null,
				// WR-05: use ?? null for token counts so a genuine 0 (e.g. :free/cached
				// requests) persists as 0 rather than collapsing to NULL ("unknown"),
				// matching the zero-vs-null discipline applied to cost_usd.
	...
	async updateUsage(
		...
		`,
			[
				usage.model || null,
				// WR-05: ?? null preserves a genuine 0 token count (see INSERT above).
				usage.promptTokens ?? null,
				usage.completionTokens ?? null,
				usage.totalTokens ?? null,
				usage.costUsd ?? null,
